...
public class FractalCreator implements FractalsCreator {
    ...
    public Fractal getFractal(String name, int step) {
        switch (name) {
            case MINKOVSKICURVE:
                return new MinkowskiCurve(step);
            case KOCHCURVE:
                return new KochCurve(step);
            case SERPINSKICARPET:
                return new SierpinskiCarpet(step);
            case SERPINSKITRIANGE:
                return new SierpinskiTriangle(step);
            case CIRCLECURVE:
                return new CircleCurve(step);
            case CIRCLESQUARE:
                return new CircleCurveSquare(step);
            case FRACTALSTREE:
                return new FractalTree(step);
